...
export const ExploreServicesTool: MCPTool = {
  ...
  execute: async ({
    ...
    logger.info(
      'Exploring available services with complete execution information...'
    );
    ...
    try {
      ...
      const servicesByCategory = serviceRegistry.groupServicesByCategory(
        registry.services
      );
      ...
      if (registry.metadata.errors.length > 0) {
        (enhancedResponse as any).errors = registry.metadata.errors;
      }
