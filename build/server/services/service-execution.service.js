...
export class ServiceExecutionService {
     ...
     */
    async executeService(request, service) {
        ...
        try {
            ...
            const signedTransaction = await this.generateSignedTransaction(service.price.amount, service.paymentInfo.walletAddress, service.price.paymentMethod);
            // Execute the make-purchase call
            const purchaseResult = await this.executePurchase(client, request.serviceId, request.params, service.price.paymentMethod, signedTransaction);
     ...
     */
    async verifyServiceAvailability(client, serviceId) {
        try {
     ...
     */
    async generateSignedTransaction(amount, walletAddress, paymentMethod) {
        try {
            this.logger.info(`Signing transaction: ${amount} ${paymentMethod} to ${walletAddress}`);
            const privateKey = this.wallet.getPrivateKey(paymentMethod);
            if (!privateKey) {
                throw new Error(`No private key found for payment method: ${paymentMethod}`);
            }
            const signedTransaction = await this.paymentsTools.signTransaction(amount, walletAddress, privateKey, "http://example.com", // TODO: Make this configurable
            paymentMethod);
     ...
     */
    async executePurchase(client, itemId, params, paymentMethod, signedTransaction) {
        try {
            const purchaseArgs = {
                itemId,
                params,
     ...
     */
    getExecutionCostEstimate(service) {
        return {
     ...
     */
    validateWalletAvailability(paymentMethod) {
        try {
            const privateKey = this.wallet.getPrivateKey(paymentMethod);
            return !!privateKey;
        }
